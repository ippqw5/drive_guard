...
class DRLGazebo(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        # Try to find the goal_box model path more robustly
        base_path = os.getenv('DRLNAV_BASE_PATH')
        if base_path:
            # Use environment variable if available
            self.entity_dir_path = os.path.join(base_path, 'src/turtlebot3_simulations/turtlebot3_gazebo/models/turtlebot3_drl_world/goal_box')
        else:
            # Fallback: try to find the path dynamically
            current_dir = os.path.dirname(os.path.realpath(__file__))
            # Go up to the workspace root and find the goal_box model
            workspace_root = current_dir
            while workspace_root != '/' and not os.path.exists(os.path.join(workspace_root, 'src')):
                workspace_root = os.path.dirname(workspace_root)
            
            if workspace_root != '/':
                self.entity_dir_path = os.path.join(workspace_root, 'src/turtlebot3_simulations/turtlebot3_gazebo/models/turtlebot3_drl_world/goal_box')
            else:
                # Last resort: try the original logic but with flexible python version
                self.entity_dir_path = current_dir
                for python_version in ['python3.10', 'python3.8', 'python3.9', 'python3.11']:
                    pattern = f'turtlebot3_drl/lib/{python_version}/site-packages/turtlebot3_drl/drl_gazebo'
                    if pattern in current_dir:
                        self.entity_dir_path = current_dir.replace(pattern, 'turtlebot3_gazebo/share/turtlebot3_gazebo/models/turtlebot3_drl_world/goal_box')
                        break
        ...
        if not os.path.exists(self.entity_path):
            raise FileNotFoundError(f"Goal box model not found at {self.entity_path}. Please check your installation.")
    ...
    *******************************************************************************"""
    ...
    def get_obstacle_coordinates(self):
        ...
        if base_path is None:
            raise EnvironmentError("DRLNAV_BASE_PATH environment variable is not set. Please source the setup.sh file.")
        
        tree = ET.parse(base_path + '/src/turtlebot3_simulations/turtlebot3_gazebo/models/turtlebot3_drl_world/inner_walls/model.sdf')
        root = tree.getroot()
